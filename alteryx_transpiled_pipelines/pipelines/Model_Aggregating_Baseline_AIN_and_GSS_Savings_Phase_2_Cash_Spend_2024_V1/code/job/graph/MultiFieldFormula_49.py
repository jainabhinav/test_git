...
def MultiFieldFormula_49(spark: SparkSession, in0: DataFrame) -> DataFrame:
    return in0.select(
        *(
          [expr("CASE WHEN CAST(isnull(`Cash Spend`) AS BOOLEAN) THEN 0 ELSE `Cash Spend` END").alias("Cash Spend"),            expr(
             ...
             .alias("Planning Account Description"),            expr("CASE WHEN CAST(isnull(`AIN_Savings`) AS BOOLEAN) THEN 0 ELSE `AIN_Savings` END").alias("AIN_Savings"),            expr("CASE WHEN CAST(isnull(`ASHB`) AS BOOLEAN) THEN 0 ELSE `ASHB` END").alias("ASHB"),            expr("CASE WHEN CAST(isnull(`Category`) AS BOOLEAN) THEN 0 ELSE `Category` END").alias("Category"),            expr("CASE WHEN CAST(isnull(`Non-Opex Adjustments`) AS BOOLEAN) THEN 0 ELSE `Non-Opex Adjustments` END")\
             ...
             .alias("Sum_Adjusted_Sum_LCL"),            expr("CASE WHEN CAST(isnull(`AIN_Resource_Cost`) AS BOOLEAN) THEN 0 ELSE `AIN_Resource_Cost` END")\
             ...
             .alias("cost center number"),            expr("CASE WHEN CAST(isnull(`gl account`) AS BOOLEAN) THEN 0 ELSE `gl account` END").alias("gl account"),            expr("CASE WHEN CAST(isnull(`evp`) AS BOOLEAN) THEN 0 ELSE `evp` END").alias("evp"),            expr(
             ...
             .alias("product description"),            expr("CASE WHEN CAST(isnull(`EW_Reduction`) AS BOOLEAN) THEN 0 ELSE `EW_Reduction` END")\
             .alias("EW_Reduction"),            expr("CASE WHEN CAST(isnull(`Sum_Adjusted_Sum_USDAFX`) AS BOOLEAN) THEN 0 ELSE `Sum_Adjusted_Sum_USDAFX` END")\
          ...
          + [
            ...
            for colName in sorted(
              (
                set(in0.columns)
                - {"Cash Spend",  "Planning Account Description",  "AIN_Savings",  "ASHB",  "Category",                                      "Non-Opex Adjustments",                                      "Planning Account Actual",                                      "Planning Account Description Actual",                                      "scenario name",  "sub mc",  "site",                                      "Sum_Adjusted_Sum_LCL",  "AIN_Resource_Cost",                                      "year",  "material group",  "OSE Labor",  "quarter",                                      "Company Code",  "Sum_Adjusted_Sum_LCLCFX",                                      "Hyperion Category",  "version",  "scenario type",                                      "Hyperion Category Code",  "planning sku",                                      "Planning Account",  "cost center number",                                      "gl account",  "evp",                                      "material group description",  "mc",  "hlmc",                                      "regrouped level 4",  "CTS v3",  "cost center",                                      "RecordID",  "product description",                                      "EW_Reduction",  "Sum_Adjusted_Sum_USDAFX",                                      "Company Description",  "gl account number"}
              )
            )
          ]
          + []
