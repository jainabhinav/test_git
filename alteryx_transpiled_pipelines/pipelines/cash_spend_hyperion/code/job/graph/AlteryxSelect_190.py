...
def AlteryxSelect_190(spark: SparkSession, in0: DataFrame) -> DataFrame:
    return in0.select(
        col("Country"), 
        ...
        col("`source_invoice spend (actual fx rate)`").cast(DoubleType()).alias("source_invoice spend (actual fx rate)"), 
        col("`source_invoice spend (budget fx rate)`").cast(DoubleType()).alias("source_invoice spend (budget fx rate)"), 
        col("`source_invoice spend (original currency)`")\
          .cast(DoubleType())\
          .alias("source_invoice spend (original currency)")
    )
