...
def Summarize_98(spark: SparkSession, in0: DataFrame) -> DataFrame:
    df1 = in0.groupBy(
    ...
    return df1.agg(
        sum(col("Adjusted_Sum_USDAFX")).alias("Sum_Adjusted_Sum_USDAFX"), 
        sum(col("Adjusted_Sum_LCLCFX")).alias("Sum_Adjusted_Sum_LCLCFX"), 
        sum(col("Adjusted_Sum_LCL")).alias("Sum_Adjusted_Sum_LCL")
    )
