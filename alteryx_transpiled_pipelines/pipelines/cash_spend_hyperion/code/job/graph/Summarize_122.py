...
def Summarize_122(spark: SparkSession, in0: DataFrame) -> DataFrame:
    df1 = in0.groupBy(
    ...
    return df1.agg(
        sum(col("Allocated_Sum_LCL")).alias("Sum_LCL"), 
        sum(col("Allocated_Sum_LCLCFX")).alias("Sum_LCLCFX"), 
        sum(col("Allocated_Sum_USDAFX")).alias("Sum_USDAFX")
    )
