...
def Summarize_54(spark: SparkSession, in0: DataFrame) -> DataFrame:
    df1 = in0.groupBy(
    ...
    return df1.agg(
        sum(col("LCL")).alias("Sum_LCL"), 
        sum(col("LCLCFX")).alias("Sum_LCLCFX"), 
        sum(col("USDAFX")).alias("Sum_USDAFX")
    )
