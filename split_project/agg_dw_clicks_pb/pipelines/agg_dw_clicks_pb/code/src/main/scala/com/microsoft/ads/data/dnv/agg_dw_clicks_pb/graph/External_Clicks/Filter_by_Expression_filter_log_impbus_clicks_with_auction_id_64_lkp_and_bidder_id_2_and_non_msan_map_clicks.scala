...
object Filter_by_Expression_filter_log_impbus_clicks_with_auction_id_64_lkp_and_bidder_id_2_and_non_msan_map_clicks {

  def apply(context: Context, in: DataFrame): DataFrame =
    in.filter(
      is_not_null(col("_lkp_log_impbus_clicks_pb_auctions_LOOKUP"))
        .and(col("bidder_id") =!= lit(2))
        .and(col("_lkp_log_msan_map_pb_click_auctions_LOOKUP").isNull)
    )
