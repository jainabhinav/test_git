...
object Join_dw_clicks_with_curator_impressions {
  ...
  def apply(context: Context, left: DataFrame, right: DataFrame): DataFrame =
    left
      ...
      .select(
        ...
        lit(0).cast(LongType).as("net_media_cost_microcents"),
        lit(0).cast(LongType).as("gross_revenue_microcents"),
        col("left.creative_id").as("creative_id"),
        col("left.region").as("geo_region"),
