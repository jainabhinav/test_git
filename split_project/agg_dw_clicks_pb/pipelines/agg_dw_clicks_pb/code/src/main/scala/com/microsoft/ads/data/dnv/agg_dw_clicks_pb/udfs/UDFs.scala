package com.microsoft.ads.data.dnv.agg_dw_clicks_pb.udfs
    ...
    udf(
      (
        ...
        while (compareTo(i, l_protocols.length) < 0) {
          if (in_protocols(convertToInt(i)) == 0) end_result = 1
          else if (in_protocols(convertToInt(i)) == 1) end_result = 2
          else if (in_protocols(convertToInt(i)) == 2) end_result = 3
          else if (in_protocols(convertToInt(i)) == 3) end_result = 7
          else if (in_protocols(convertToInt(i)) == 4) end_result = 8
          else end_result = 0
...
object PipelineInitCode extends Serializable {
  ...
  def f_get_auction_events_pricing_terms_for_term_id(
    in_pricing_term: org.apache.spark.sql.Column,
    in_term_id:      org.apache.spark.sql.Column,
    in_is_deduction: org.apache.spark.sql.Column
  ) =
    when(
      is_not_null(in_pricing_term),
      lit(0.0d) + aggregate(
        filter(in_pricing_term,
               ii =>
                 (ii.getField("term_id") === in_term_id)
                   .and(ii.getField("is_deduction") === in_is_deduction)
        ),
        lit(0.0d).cast(DoubleType),
        (acc, ii) => acc + ii.getField("amount")
      )
    ).otherwise(lit(0.0d))

  def f_exchange_traded_is_console_event_unit_of_trx(
    aggregation_type:    org.apache.spark.sql.Column,
    imp_type:            org.apache.spark.sql.Column,
    buyer_trx_event_id:  org.apache.spark.sql.Column,
    seller_trx_event_id: org.apache.spark.sql.Column
  ) = {
    var l_buyer_trx_event_id: org.apache.spark.sql.Column =
      coalesce(buyer_trx_event_id, lit(0))
    var l_seller_trx_event_id: org.apache.spark.sql.Column =
      coalesce(seller_trx_event_id, lit(0))
    var l_required_trx_event_for_owning_member: org.apache.spark.sql.Column =
      lit(0)
    var l_implied_trx_event_id_for_agg: org.apache.spark.sql.Column = lit(0)
    l_required_trx_event_for_owning_member =
      when(coalesce(imp_type, lit(0)) === lit(6), l_seller_trx_event_id)
        .when(coalesce(imp_type, lit(0)) === lit(7), l_buyer_trx_event_id)
        .otherwise(lit(1))
    l_implied_trx_event_id_for_agg =
      when(coalesce(aggregation_type, lit(0)) === lit(2), lit(3))
        .when(coalesce(aggregation_type, lit(0)) === lit(3), lit(4))
        .otherwise(lit(0))
    (l_required_trx_event_for_owning_member =!= lit(0))
      .and(l_implied_trx_event_id_for_agg =!= lit(0))
      .and(
        l_required_trx_event_for_owning_member === l_implied_trx_event_id_for_agg
      )
  }

  def f_update_charge_terms(
    in_member_pricing_term_new: org.apache.spark.sql.Column,
    in_member_pricing_term_old: org.apache.spark.sql.Column
  ) =
    when(
      is_not_null(in_member_pricing_term_new),
      when(
        is_not_null(in_member_pricing_term_new.getField("pricing_terms")),
        temp3629519_UDF(
          struct(
            when(isnull(in_member_pricing_term_old),
                 in_member_pricing_term_new.getField("rate_card_id")
            ).otherwise(
                when(isnull(in_member_pricing_term_old),
                     in_member_pricing_term_new
                ).otherwise(in_member_pricing_term_old).getField("rate_card_id")
              )
              .as("rate_card_id"),
            when(isnull(in_member_pricing_term_old),
                 in_member_pricing_term_new.getField("member_id")
            ).otherwise(
                when(isnull(in_member_pricing_term_old),
                     in_member_pricing_term_new
                ).otherwise(in_member_pricing_term_old).getField("member_id")
              )
              .as("member_id"),
            when(isnull(in_member_pricing_term_old),
                 in_member_pricing_term_new.getField("is_dw")
            ).otherwise(
                when(isnull(in_member_pricing_term_old),
                     in_member_pricing_term_new
                ).otherwise(in_member_pricing_term_old).getField("is_dw")
              )
              .as("is_dw"),
            when(
              is_not_null(in_member_pricing_term_new)
                .and(
                  is_not_null(
                    in_member_pricing_term_new.getField("pricing_terms")
                  )
                )
                .and(
                  isnull(
                    when(isnull(in_member_pricing_term_old),
                         in_member_pricing_term_new
                    ).otherwise(in_member_pricing_term_old)
              ...
              .otherwise(in_member_pricing_term_old.getField("pricing_terms"))
              .as("pricing_terms"),
            when(isnull(in_member_pricing_term_old),
                 in_member_pricing_term_new.getField("fx_margin_rate_id")
            ).otherwise(
                when(isnull(in_member_pricing_term_old),
                     in_member_pricing_term_new
                ).otherwise(in_member_pricing_term_old)
            ...
            when(isnull(in_member_pricing_term_old),
                 in_member_pricing_term_new.getField("marketplace_owner_id")
            ).otherwise(
                when(isnull(in_member_pricing_term_old),
                     in_member_pricing_term_new
                ).otherwise(in_member_pricing_term_old)
            ...
            when(isnull(in_member_pricing_term_old),
                 in_member_pricing_term_new.getField("virtual_marketplace_id")
            ).otherwise(
                when(isnull(in_member_pricing_term_old),
                     in_member_pricing_term_new
                ).otherwise(in_member_pricing_term_old)
            ...
            when(isnull(in_member_pricing_term_old),
                 in_member_pricing_term_new.getField("amino_enabled")
            ).otherwise(
                when(isnull(in_member_pricing_term_old),
                     in_member_pricing_term_new
                ).otherwise(in_member_pricing_term_old)
          ...
          lit(0),
          in_member_pricing_term_new
        ).getField("l_member_pricing_term_old")
      ).otherwise(
        struct(
          when(isnull(in_member_pricing_term_old),
               in_member_pricing_term_new.getField("rate_card_id")
          ...
          when(isnull(in_member_pricing_term_old),
              ...
              when(isnull(in_member_pricing_term_old),
                   in_member_pricing_term_new
              ).otherwise(in_member_pricing_term_old).getField("amino_enabled")
            )
            .as("amino_enabled")
        )
      )
      ...
      struct(
        when(isnull(in_member_pricing_term_old),
            ...
            when(isnull(in_member_pricing_term_old), in_member_pricing_term_new)
              .otherwise(in_member_pricing_term_old)
  ...
  lazy val temp3629519_UDF = udf(
    (
      _l_member_pricing_term_old: Row,
      _i:                         Integer,
      _l_member_pricing_term_new: Row
    ) => {
      var l_member_pricing_term_old = _l_member_pricing_term_old
      var i                         = _i
      var l_member_pricing_term_new = _l_member_pricing_term_new
      while (
        compareTo(i,
                  l_member_pricing_term_new
                    .getAs[Seq[Row]]("pricing_terms")
                    .toArray[Row]
                    .length
        ) < 0
      ) {
        var _pricing_terms = l_member_pricing_term_old
          .getAs[Seq[Row]]("pricing_terms")
          .toArray[Row]
          .toArray
        var _pricing_terms_i = _pricing_terms(convertToInt(i))
        if (
          _isnull(
            l_member_pricing_term_old
              .getAs[Seq[Row]]("pricing_terms")
              .toArray[Row]
              .array(convertToInt(i))
              .getAs[Integer]("term_id")
          )
        )
          _pricing_terms_i = l_member_pricing_term_new
            .getAs[Seq[Row]]("pricing_terms")
            .toArray[Row]
            .array(convertToInt(i))
        else {
          _pricing_terms_i = updateIndexInRow(
            _pricing_terms_i,
            0,
            if (
              !_isnull(
        ...
        _pricing_terms(i) = _pricing_terms_i
        l_member_pricing_term_old =
          updateIndexInRow(l_member_pricing_term_old, 3, _pricing_terms.toSeq)
        i = i + convertToInt(1)
      }
      Row(
        if (!_isnull(l_member_pricing_term_old)) {
          Row(
            convertToInt(l_member_pricing_term_old.get(0)),
            convertToInt(l_member_pricing_term_old.get(1)),
            l_member_pricing_term_old.get(2),
            l_member_pricing_term_old
              .getAs[Seq[Row]](3)
              .toArray[Row]
              .map { x =>
              ...
              .toArray,
            convertToInt(l_member_pricing_term_old.get(4)),
            convertToInt(l_member_pricing_term_old.get(5)),
            convertToInt(l_member_pricing_term_old.get(6)),
            l_member_pricing_term_old.get(7)
          )
        } else null,
        convertToInt(i)
      )
  ...
  def f_keep_data_charge(
    cost_pct:     org.apache.spark.sql.Column,
    agg_type:     org.apache.spark.sql.Column,
    payment_type: org.apache.spark.sql.Column
  ) = {
    var l_agg_type: org.apache.spark.sql.Column = coalesce(agg_type, lit(0))
  ...
  def f_payment_type_matches(
    ...
    l_payment_type_matches = when(
      ...
      lit(1)
    ).otherwise(l_payment_type_matches)
    l_payment_type_matches
  }

  lazy val temp562910_UDF = udf(
    (_l_member_pricing_term: Row, _i: Integer) => {
      var i                     = _i
      var l_member_pricing_term = _l_member_pricing_term
      while (
        compareTo(i,
                  l_member_pricing_term
                    .getAs[Seq[Row]]("pricing_terms")
                    .toArray[Row]
                    .length
        ) < 0
      ) {
        var _pricing_terms = l_member_pricing_term
          .getAs[Seq[Row]]("pricing_terms")
          .toArray[Row]
          .toArray
        var _pricing_terms_i = _pricing_terms(convertToInt(i))
        _pricing_terms_i = updateIndexInRow(_pricing_terms_i, 1, 0.0d)
        _pricing_terms(i) = _pricing_terms_i
        l_member_pricing_term =
          updateIndexInRow(l_member_pricing_term, 3, _pricing_terms.toSeq)
        i = i + convertToInt(1)
      }
      Row(
        convertToInt(i),
        if (!_isnull(l_member_pricing_term)) {
          Row(
            convertToInt(l_member_pricing_term.get(0)),
            convertToInt(l_member_pricing_term.get(1)),
            l_member_pricing_term.get(2),
            l_member_pricing_term
              .getAs[Seq[Row]](3)
              .toArray[Row]
              .map { x =>
              ...
              .toArray,
            convertToInt(l_member_pricing_term.get(4)),
            convertToInt(l_member_pricing_term.get(5)),
            convertToInt(l_member_pricing_term.get(6)),
            l_member_pricing_term.get(7)
          )
        } else null
      )
    },
  ...
  def f_create_empty_pricing_term(member_id: org.apache.spark.sql.Column) = {
    var l_member_id: org.apache.spark.sql.Column = member_id
    var l_member_pricing_term: org.apache.spark.sql.Column = lit(null).cast(
      StructType(
        List(
          StructField("rate_card_id", IntegerType, true),
          StructField("member_id",    IntegerType, true),
    ...
    var l_is_dw: org.apache.spark.sql.Column = lit(1).cast(BooleanType)
    l_member_pricing_term = struct(
      lit(null).cast(IntegerType).as("rate_card_id"),
      when(is_not_null(member_id), l_member_id)
        .otherwise(lit(null).cast(IntegerType))
        .as("member_id"),
      when(is_not_null(member_id), l_is_dw)
  ...
  def f_zero_out_pricing_terms(
    in_member_pricing_term: org.apache.spark.sql.Column
  ) =
    when(
      is_not_null(in_member_pricing_term),
      when(is_not_null(in_member_pricing_term.getField("pricing_terms")),
           temp562910_UDF(in_member_pricing_term, lit(0)).getField(
             "l_member_pricing_term"
           )
      ).otherwise(in_member_pricing_term)
    ).otherwise(in_member_pricing_term)
  ...
  def f_is_matching_payment_and_agg_type(
    ...
    l_is_matching_payment_and_agg_type = when(
      (coalesce(agg_type, lit(0)) =!= lit(1))
        .and(coalesce(agg_type, lit(0)) =!= lit(2)),
  ...
  def f_clicks_pass_through_financials(
    in_transactionEventInfo:   org.apache.spark.sql.Column,
    in_revenueInfo:            org.apache.spark.sql.Column,
    in_imp_type:               org.apache.spark.sql.Column,
    in_payment_type:           org.apache.spark.sql.Column,
    in_revenue_type:           org.apache.spark.sql.Column,
    in_media_buy_revshare_pct: org.apache.spark.sql.Column,
    in_commissions_revshare:   org.apache.spark.sql.Column,
    in_serving_fees_revshare:  org.apache.spark.sql.Column,
    in_sales_tax_rate_pct:     org.apache.spark.sql.Column
  ) = {
    var l_transactionEventInfo: org.apache.spark.sql.Column =
      when(is_not_null(in_transactionEventInfo), in_transactionEventInfo)
        .otherwise(
          struct(
    ...
    var l_revenueInfo: org.apache.spark.sql.Column =
      when(is_not_null(in_revenueInfo), in_revenueInfo).otherwise(
        struct(
    ...
    var l_imp_type: org.apache.spark.sql.Column = coalesce(in_imp_type, lit(1))
    var l_payment_type: org.apache.spark.sql.Column =
      coalesce(in_payment_type, lit(999))
    var l_revenue_type: org.apache.spark.sql.Column =
      coalesce(in_revenue_type, lit(999))
    var co: org.apache.spark.sql.Column = struct(
      lit(null).as("total_partner_fees_microcents"),
      lit(null).as("booked_revenue_dollars"),
      lit(null).as("booked_revenue_adv_curr"),
      lit(null).as("total_data_costs_microcents"),
      ...
      lit(null).as("seller_revenue"),
      lit(null).as("net_media_cost_dollar_cpm"),
      lit(null).as("media_cost_dollar_cpm"),
      lit(null).as("buyer_transacted"),
      lit(null).as("seller_transacted"),
      lit(null).as("is_buy_side_imp"),
      lit(null).as("is_sell_side_imp"),
      lit(null).as("buyer_member_pricing_term"),
      lit(null).as("seller_member_pricing_term")
    )
    var micro_cents_to_cpm: org.apache.spark.sql.Column = lit(0.000010d)
    var l_booked_revenue_adv_curr: org.apache.spark.sql.Column =
      when(l_revenueInfo.getField("booked_revenue_adv_curr") > lit(0.0d),
           l_revenueInfo.getField("booked_revenue_adv_curr")
      ).otherwise(lit(0.0d))
    var l_booked_revenue_dollars: org.apache.spark.sql.Column =
      when(l_revenueInfo.getField("booked_revenue_dollars") > lit(0.0d),
           l_revenueInfo.getField("booked_revenue_dollars")
      ).otherwise(lit(0.0d))
    var l_total_partner_fees_mircocents: org.apache.spark.sql.Column =
      coalesce(l_revenueInfo.getField("total_partner_fees_microcents"), lit(0))
    var l_total_data_costs_microcents: org.apache.spark.sql.Column =
      coalesce(l_revenueInfo.getField("total_data_costs_microcents"), lit(0))
    var l_total_profit_microcents: org.apache.spark.sql.Column =
      coalesce(l_revenueInfo.getField("total_profit_microcents"), lit(0))
    var l_buyer_transacted: org.apache.spark.sql.Column =
      l_transactionEventInfo.getField("buyer_transacted").cast(BooleanType)
    var l_seller_transacted: org.apache.spark.sql.Column =
      l_transactionEventInfo.getField("seller_transacted").cast(BooleanType)
    var l_payment_value: org.apache.spark.sql.Column =
      when(l_buyer_transacted.cast(BooleanType),
           micro_cents_to_cpm * l_transactionEventInfo.getField(
             "gross_payment_value_microcents"
           )
      ).otherwise(lit(0.0d))
    var l_seller_revenue: org.apache.spark.sql.Column =
      when(l_seller_transacted.cast(BooleanType),
           micro_cents_to_cpm * l_transactionEventInfo.getField(
             "seller_revenue_microcents"
           )
      ).otherwise(lit(0.0d))
    var l_net_media_cost_dollar_cpm: org.apache.spark.sql.Column =
      when(l_buyer_transacted.cast(BooleanType),
           micro_cents_to_cpm * l_transactionEventInfo.getField(
             "net_payment_value_microcents"
           )
      ).otherwise(lit(0.0d))
    var is_buy_side_imp: org.apache.spark.sql.Column =
      when(l_imp_type.isin(lit(10), lit(9), lit(11), lit(7), lit(5)),
           lit(1).cast(BooleanType)
      ).otherwise(lit(0).cast(BooleanType))
    var is_sell_side_imp: org.apache.spark.sql.Column = when(
      l_imp_type.isin(lit(8), lit(2), lit(3), lit(4), lit(1), lit(5), lit(6)),
      lit(1).cast(BooleanType)
    ).otherwise(lit(0).cast(BooleanType))
    var l_media_buy_revshare_pct: org.apache.spark.sql.Column =
      coalesce(in_media_buy_revshare_pct, lit(0))
    var l_commissions_revshare: org.apache.spark.sql.Column =
      coalesce(in_commissions_revshare, lit(0))
    var l_serving_fees_revshare: org.apache.spark.sql.Column =
      coalesce(in_serving_fees_revshare, lit(0))
    var l_media_cost_dollars_cpm: org.apache.spark.sql.Column = lit(0.0d)
    var dollars_to_cpm:           org.apache.spark.sql.Column = lit(1000.0d)
    l_media_cost_dollars_cpm = when(coalesce(in_imp_type, lit(1)) === lit(7),
                                    when(l_imp_type === lit(7), l_payment_value)
    ).when(
        coalesce(in_imp_type, lit(1)) === lit(6),
        when((l_payment_type === lit(4)).and(l_revenue_type === lit(3)),
             when(l_imp_type === lit(6),
                  l_seller_revenue * l_media_buy_revshare_pct
             )
        ).otherwise(l_media_cost_dollars_cpm)
      )
      .when(
        (coalesce(in_imp_type, lit(1)) === lit(5))
          .and(coalesce(in_media_buy_revshare_pct, lit(0)) > lit(0)),
        dollars_to_cpm * (l_media_buy_revshare_pct * ((lit(
          1
        ) - l_commissions_revshare) * when(is_buy_side_imp.cast(BooleanType),
                                           l_booked_revenue_dollars
        ).otherwise(lit(0.0d))) / (lit(1) + l_serving_fees_revshare))
      )
      .otherwise(lit(0.0d))
    co = struct(
      when(is_buy_side_imp.cast(BooleanType), l_total_partner_fees_mircocents)
        .otherwise(lit(0))
        .as("total_partner_fees_microcents"),
      when(is_buy_side_imp.cast(BooleanType), l_booked_revenue_dollars)
        .otherwise(lit(0.0d))
        .as("booked_revenue_dollars"),
      when(is_buy_side_imp.cast(BooleanType), l_booked_revenue_adv_curr)
        .otherwise(lit(0.0d))
        .as("booked_revenue_adv_curr"),
      when(is_buy_side_imp.cast(BooleanType), l_total_data_costs_microcents)
        .otherwise(lit(0))
        .as("total_data_costs_microcents"),
      when(is_buy_side_imp.cast(BooleanType), l_total_profit_microcents)
        .otherwise(lit(0))
        .as("total_profit_microcents"),
      when(l_imp_type === lit(7), l_payment_value)
        .otherwise(lit(0.0d))
        .as("payment_value"),
      when(l_imp_type === lit(6), l_seller_revenue)
        .otherwise(lit(0.0d))
        .as("seller_revenue"),
      when(l_imp_type === lit(7), l_net_media_cost_dollar_cpm)
        .otherwise(lit(0.0d))
        .as("net_media_cost_dollar_cpm"),
      l_media_cost_dollars_cpm.as("media_cost_dollar_cpm"),
      l_buyer_transacted.as("buyer_transacted"),
      l_seller_transacted.as("seller_transacted"),
      is_buy_side_imp.as("is_buy_side_imp"),
      is_sell_side_imp.as("is_sell_side_imp"),
      lit(null).as("buyer_member_pricing_term"),
      lit(null).as("seller_member_pricing_term")
    )
    co
