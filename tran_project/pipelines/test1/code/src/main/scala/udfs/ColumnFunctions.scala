...
object ColumnFunctions extends Serializable {
  ...
  def f_get_transaction_event_pricing(
    ...
    var l_transaction_event_pricing = impression_transaction_event_pricing
    var ltep_gross_payment_value_microcents: Column = when(
    ...
    var ltep_net_payment_value_microcents: Column = when(
      (should_process_views === lit(1))
        .and(
          is_not_null(
            auction_transaction_event_pricing.getField("buyer_transacted")
      ...
      auction_transaction_event_pricing.getField("net_payment_value_microcents")
    ).otherwise(
      impression_transaction_event_pricing
        .getField("net_payment_value_microcents")
    )
    var ltep_seller_revenue_microcents: Column = when(
      (should_process_views === lit(1))
        .and(
          is_not_null(
      ...
      impression_transaction_event_pricing.getField("seller_revenue_microcents")
    )
    var ltep_buyer_transacted: Column = when(
      (should_process_views === lit(1))
        .and(
          is_not_null(
            auction_transaction_event_pricing.getField("buyer_transacted")
    ...
    var ltep_seller_transacted: Column = when(
      (should_process_views === lit(1))
        .and(
          is_not_null(
            auction_transaction_event_pricing.getField("seller_transacted")
    ...
    val ltep_buyer_charges: Column = when(
      (should_process_views === lit(1))
        .and(
          is_not_null(
            auction_transaction_event_pricing.getField("buyer_transacted")
    ...
    val ltep_seller_charges: Column = when(
      (should_process_views === lit(1))
        .and(
          is_not_null(
            auction_transaction_event_pricing.getField("seller_transacted")
    ...
    ).otherwise(impression_transaction_event_pricing.getField("seller_charges"))
    val ltep_buyer_charges2: Column = when(
      is_not_null(buyer_charges),
      struct(
    ...
    val ltep_seller_charges2: Column = when(
      is_not_null(seller_charges),
      struct(
    ...
    struct(
      ...
      when(is_not_null(ltep_buyer_transacted)
             .and(ltep_buyer_transacted =!= lit(1))
             .and(ltep_buyer_charges2.getField("pricing_terms").isNull),
           buyer_charges
      ).otherwise(ltep_buyer_charges2).as("buyer_charges"),
      when(is_not_null(ltep_seller_transacted)
             .and(ltep_seller_transacted =!= lit(1))
             .and(ltep_seller_charges2.getField("pricing_terms").isNull),
           seller_charges
      ).otherwise(ltep_seller_charges2).as("seller_charges"),
  ...
  def f_view_non_measurable_reason(
    ...
    when(view_detection_enabled === lit(0),               lit(1))
  ...
  def f_view_result(
    ...
    when(is_not_null(impbus_view_result).and(view_detection_enabled === lit(1)),
         impbus_view_result
  ...
  def f_should_process_views(
    ...
    when(
      is_not_null(in_log_dw_view)
        .and(coalesce(seller_trx_event_id, lit(0)).isin(0, 1))
        .and(
          coalesce(buyer_trx_event_id, lit(0)).isin(2, 5, 6, 7, 8, 9, 13, 14,
            15,                        16)
        ),
      lit(1)
    ).otherwise(lit(0)).cast(IntegerType)
  ...
  def f_view_measurable(
    ...
    when(is_not_null(impbus_view_result)
  ...
  def f_should_zero_seller_revenue(
    ...
    when(
      (coalesce(imp_type, lit(0)) === lit(5))
        ...
        .and(
          coalesce(seller_member_id, lit(0))
            .isin(280,               14007, 14120, 14136, 14634)
        ),
      lit(1)
    ).otherwise(
        when(
          (coalesce(imp_type, lit(0)) === lit(4))
            ...
            .and(
              when(is_not_null(bid).cast(BooleanType),
  ...
  def f_viewable(view_measurable: Column, impbus_view_result: Column): Column =
    when((view_measurable === lit(1))
  ...
  def f_get_curator_margin(pricing_terms: Column): Column =
    when(is_not_null(pricing_terms).cast(BooleanType),
         temp692552_UDF(lit(0), lit(0.0d), lit(0), pricing_terms, lit(0))
           .getField("l_curator_margin")
    ).otherwise(lit(0))

  def f_get_total_tech_fees(pricing_terms: Column): Column =
    when(is_not_null(pricing_terms).cast(BooleanType),
         temp1433148_UDF(lit(0.0d), lit(0), lit(0), pricing_terms, lit(0))
           .getField("l_total_tech_fees")
    ).otherwise(lit(0))

  def f_get_seller_fees(pricing_terms: Column): Column =
    when(is_not_null(pricing_terms).cast(BooleanType),
         temp2173548_UDF(lit(0), lit(0.0d), lit(0), pricing_terms, lit(0))
  ...
  def f_is_error_imp(imp_type: Column): Column =
    when(coalesce(imp_type, lit(0)).isin(1, 3, 8), lit(1))
      .otherwise(lit(0))
      .cast(IntegerType)
  ...
  def f_is_non_cpm_payment_or_payment(
    ...
    when((coalesce(imp_type, lit(0)) === lit(7)).and(
           coalesce(payment_type, lit(999)).isin(1, 2, 5)
         ),
         lit(1)
    ).when((coalesce(imp_type, lit(0)) === lit(6))
              .and(coalesce(revenue_type, lit(999)).isin(3, 4, 9)),
            lit(1)
      )
      .otherwise(lit(0))
      .cast(IntegerType)
  ...
  def f_get_media_cost_dollars_cpm(
    ...
    when(coalesce(imp_type, lit(0)) === lit(2),
         when(l_media_buy_rev_share_pct === lit(0), l_media_buy_cost).otherwise(
           lit(0)
         )
  ...
  def f_create_agg_dw_impressions(
    ...
    _f_is_default_or_error_imp:        Column,
    _f_is_error_imp:                   Column,
    should_process_views:              Column
  ): Column = {
    ...
    val buyer_charges_pricing_terms3 = when(
    ...
    val seller_charges_pricing_terms3 = when(
  ...
  def f_create_agg_dw_impressions_return_11668614(
    ...
    val virtual_log_dw_bid_creative_freq = when(
      _f_is_buy_side === lit(0),
      when(
        isnull(col("log_dw_bid")).and(
          (_f_is_error_imp === lit(1)).or(
    ...
    struct(
      ...
      coalesce(
        ...
        when(_f_is_buy_side === lit(0), virtual_log_dw_bid_creative_freq)
      ).as("creative_freq"),
      coalesce(
        when((_f_is_buy_side === lit(1)).and(
               col("log_dw_bid.creative_rec").cast(IntegerType) =!= lit(0)
        ...
        when(_f_is_buy_side === lit(0), virtual_log_dw_bid_creative_rec)
      ).as("creative_rec"),
      coalesce(
        when((_f_is_buy_side === lit(1)).and(
               col("log_dw_bid.predict_type").cast(IntegerType) === lit(1)
      ...
      coalesce(
        ...
        when((_f_is_buy_side === lit(1)).and(imp_type === lit(9)), lit(0)),
        when(_f_is_buy_side === lit(0),                            virtual_log_dw_bid_advertiser_freq)
      ).as("advertiser_frequency"),
      coalesce(
        when((_f_is_buy_side === lit(1)).and(
               col("log_dw_bid.advertiser_rec").cast(IntegerType) =!= lit(0)
        ...
        when((_f_is_buy_side === lit(1)).and(imp_type === lit(9)), lit(0)),
        when(_f_is_buy_side === lit(0),                            virtual_log_dw_bid_advertiser_rec)
      ).as("advertiser_recency"),
      coalesce(
        when(col("log_impbus_impressions.user_id_64").cast(LongType) > lit(
               9223372036854775807L
      ...
      coalesce(
        ...
        when(_f_is_buy_side === lit(0), virtual_log_dw_bid_pricing_type)
      ).as("pricing_type"),
      when((_f_is_buy_side === lit(1))
             .and(col("log_dw_bid.can_convert").cast(IntegerType) =!= lit(0)),
           col("log_dw_bid.can_convert").cast(IntegerType)
      ...
      coalesce(when(_f_is_buy_side === lit(1),
               ...
               when(_f_is_buy_side === lit(0), virtual_log_dw_bid_predict_type)
      ).as("predict_type_rev"),
      coalesce(
        when((_f_is_buy_side === lit(1)).and(
               col("log_dw_bid.predict_type_goal").cast(IntegerType) =!= lit(0)
        ...
        when(_f_is_buy_side === lit(0), virtual_log_dw_bid_predict_type_goal)
      ).as("predict_type_goal"),
      coalesce(when(_f_is_buy_side === lit(1),
                    col("log_dw_bid.predict_type").cast(IntegerType)
               ),
      ...
      coalesce(
        ...
        when((_f_is_buy_side === lit(0))
               .and(virtual_log_dw_bid_campaign_group_freq =!= lit(-2)),
      ...
      coalesce(
        ...
        when(_f_is_buy_side === lit(0), virtual_log_dw_bid_campaign_group_rec)
      ).as("campaign_group_rec"),
      coalesce(
        when(
          (_f_is_buy_side === lit(1)).and(
        ...
        when((_f_is_buy_side === lit(0))
               .and(virtual_log_dw_bid_insertion_order_freq =!= lit(-2)),
      ...
      coalesce(
        ...
        when(_f_is_buy_side === lit(0), virtual_log_dw_bid_insertion_order_rec)
      ).as("insertion_order_rec"),
      when((_f_is_buy_side === lit(1)).and(
             string_compare(col("log_dw_bid.buyer_gender"), lit("u")) =!= lit(0)
           ),
      ...
      coalesce(
        when((_f_is_buy_side === lit(0))
               .and(virtual_log_dw_bid_revenue_auction_event_type =!= lit(0)),
