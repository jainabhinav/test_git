  ...
  def apply(context: Context, left: DataFrame, right: DataFrame): DataFrame =
    left
      ...
      .select(
          ...
          coalesce(
            when(
              is_not_null(
                ...
                .and(
                  col(
                    "right.log_impbus_impressions_pricing.buyer_charges.is_dw"
                  ).cast(ByteType) === lit(1)
                ),
              col("right.log_impbus_impressions_pricing.buyer_charges.is_dw")
                .cast(IntegerType)
            ...
            when(
              is_not_null(
                ...
                .and(
                  col(
                    "right.log_impbus_impressions_pricing.seller_charges.is_dw"
                  ).cast(ByteType) === lit(1)
                ),
              col("right.log_impbus_impressions_pricing.seller_charges.is_dw")
                .cast(IntegerType)
        ...
        coalesce(col("left.is_transactable"),          lit(false)).as("is_transactable"),
        coalesce(col("left.is_transacted_previously"), lit(false))
          .as("is_transacted_previously"),
        coalesce(col("left.is_deferred_impression"), lit(false))
          .as("is_deferred_impression"),
        coalesce(col("left.has_null_bid"), lit(false)).as("has_null_bid"),
