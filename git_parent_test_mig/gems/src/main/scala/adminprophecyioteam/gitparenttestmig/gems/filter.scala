package adminprophecyioteam.gitparenttestmig.gems
...
class Filter extends ComponentSpec {
  ...
  val gemDescription: String = "Filters rows of the DataFrame based on the provided filter conditions"
  ...
  def dialog: Dialog = Dialog("Filter")
    .addElement(
      ColumnsLayout(height = Some("100%"))
        .addColumn(
          Ports(selectedFieldsProperty = Some("columnsSelector")),
          "content"
        )
        .addColumn(
  ...
  def validate(component: Component)(implicit context: WorkflowContext): List[Diagnostic] = {
    val testColumnPresence = context match {
      case ValidateSchemaWorkflowContext => Some(ColumnsUsage.WithoutInputAlias)
      case _ => None
    }

    val diagnostics = validateSColumn(component.properties.condition, "condition", component, testColumnPresence)
    diagnostics.toList
  }
