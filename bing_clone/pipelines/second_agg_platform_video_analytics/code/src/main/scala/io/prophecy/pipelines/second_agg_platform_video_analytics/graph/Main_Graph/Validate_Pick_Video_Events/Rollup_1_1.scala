...
object Rollup_1_1 {
  ...
  def apply(context: Context, in: DataFrame): DataFrame =
    in.groupBy(col("auction_id_64"))
      .agg(
        rtb_imp_type(context).as("rtb_imp_type"),
        resold_imp_type(context).as("resold_imp_type"),
        rtb_request_imp_type(context).as("rtb_request_imp_type"),
        resold_request_imp_type(context).as("resold_request_imp_type"),
        others_imp_type(context).as("others_imp_type"),
    ...
    first(
      when(
        ...
        struct(
          ...
          col("request_imp_type").cast(IntegerType).as("request_imp_type"),
          col("call_type"),
  ...
  def others_request_imp_type(context: Context) = {
    val spark  = context.spark
    val Config = context.config
    coalesce(
      first(
        when(
          (coalesce(col("request_imp_type").cast(IntegerType), lit(0)) =!= lit(
            6
          )).and(
              coalesce(col("request_imp_type").cast(IntegerType),
                       lit(0)
              ) =!= lit(7)
            )
            .and(
  ...
  def rtb_imp_type(context: Context) = {
    val spark  = context.spark
    val Config = context.config
    first(
      when(
