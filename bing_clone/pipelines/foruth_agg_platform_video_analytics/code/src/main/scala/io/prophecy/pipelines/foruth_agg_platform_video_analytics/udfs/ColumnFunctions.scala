package io.prophecy.pipelines.foruth_agg_platform_video_analytics.udfs
...
object ColumnFunctions extends Serializable {
  ...
  def f_cal_protocals_bitmap(in_protocols: Column): Column =
    aggregate(filter(coalesce(in_protocols, array()), xx => xx <= lit(31)),
              lit(0),
              (xx, ji) => xx.bitwiseOR(pow(lit(2), ji).cast(IntegerType))
    )

  def f_is_vpaid(in_frameworks: Column): Column =
    element_at(
      reverse(
        filter(
          transform(coalesce(split(in_frameworks, ","), array()),
                    ji =>
                      when((rtrim(ji) === lit("1")).or(rtrim(ji) === lit("2")),
                           lit(1).cast(BooleanType)
                      )
          ),
          xx => is_not_null(xx)
  ...
  def f_map_video_protocol_enum(in_protocols: Column): Column =
    temp2408380_UDF(coalesce(in_protocols, array()),
                    coalesce(in_protocols, array()),
                    lit(0),
                    lit(0),
                    array().cast("array<int>")
    ).getField("in_video_protocol_enum")

  def f_is_curr_hour(
    in_date_time:                                 Column,
    global_variable_substituted_XR_BUSINESS_DATE: Column,
    global_variable_substituted_XR_BUSINESS_HOUR: Column
  ): Column =
    in_date_time >= unix_timestamp(
      date_format(
        to_timestamp(concat(global_variable_substituted_XR_BUSINESS_DATE,
  ...
  def f_get_winning_creative_id(l_ve: Column, l_vi: Column): Column = {
    var l_winning_creative_id: org.apache.spark.sql.Column = lit(0)
    ...
    l_winning_creative_id
  }

  def f_is_buy_side_imp(l_imp_type: Column): Column = {
  ...
  def f_calc_financials(
    ...
    var l_fin_booked_revenue_dollars: org.apache.spark.sql.Column =
      lit(null).cast(DoubleType)
    ...
    var l_booked_revenue_dollars: org.apache.spark.sql.Column = lit(0.0d)
    var l_media_cost_dollar_cpm:  org.apache.spark.sql.Column = lit(0.0d)
    var l_serving_fees:           org.apache.spark.sql.Column = lit(0.0d)
    var l_commissions:            org.apache.spark.sql.Column = lit(0.0d)
    var l_reseller_revenue:       org.apache.spark.sql.Column = lit(0.0d)
    ...
    l_booked_revenue_dollars = when(
      ...
      coalesce(l_ve.getField("booked_revenue_dollars"), lit(0.0d))
    ).otherwise(l_booked_revenue_dollars).cast(DoubleType)
    l_media_cost_dollar_cpm = when(
      ...
      coalesce(l_ve.getField("media_cost_dollars_cpm") / lit(1000), lit(0.0d))
    ).otherwise(l_media_cost_dollar_cpm).cast(DoubleType)
    l_serving_fees = when(
      is_not_null(l_ve.getField("date_time")).and(
        f_is_curr_hour(l_ve.getField("date_time"),
    ...
    l_booked_revenue_dollars = when(
      ...
      l_booked_revenue_dollars + coalesce(
        l_vi.getField("booked_revenue_dollars"),
        lit(0.0d)
      )
    ).otherwise(l_booked_revenue_dollars).cast(DoubleType)
    l_media_cost_dollar_cpm = when(
    ...
    l_serving_fees = when(
      ...
      (coalesce(l_vi.getField("serving_fees_cpm"), lit(0.0d)) + coalesce(
    ...
    l_commissions = when(
      ...
      coalesce(l_vi.getField("commission_cpm") / lit(1000),
               lit(0.0d)
      ) + coalesce(l_vi.getField("commission_revshare") * l_vi.getField(
                     "booked_revenue_dollars"
                   ),
                   lit(0.0d)
      )
    ...
    l_reseller_revenue = when(
      ...
      coalesce(l_vi.getField("seller_revenue"), lit(0.0d))
    ).otherwise(l_reseller_revenue).cast(DoubleType)
    l_booked_revenue_dollars = when(
    ...
    l_serving_fees = when(
      ...
      coalesce(l_serving_fees, lit(0.0d)) + coalesce(
    ...
    l_commissions = when(
      ...
      coalesce(l_commissions, lit(0.0d)) + coalesce(
  ...
  def f_currency_mapper(
    in_fx_rate_snapshot_id:            Column,
    in_buyer_member_id:                Column,
    in_seller_member_id:               Column,
    in_advertiser_id:                  Column,
    in_imp_type:                       Column,
    sup_bidder_advertiser_pb_lookup_2: Column,
    sup_bidder_advertiser_pb_lookup:   Column
  ): Column = {
    var l_currency_advertiser_default_exchange_rate
      : org.apache.spark.sql.Column = lit(null).cast(DoubleType)
    ...
    var l_imp_type_0: org.apache.spark.sql.Column =
      in_imp_type.cast(IntegerType)
    ...
    var l_advertiser_default_currency_0: org.apache.spark.sql.Column = rpad(
      coalesce(
        ...
        sup_bidder_advertiser_pb_lookup_2.getField(
          "advertiser_default_currency"
        ),
        lit("USD")
      ),
    ...
    var l_advertiser_default_exchange_rate_0: org.apache.spark.sql.Column =
      coalesce(
        ...
        lookup("sup_sup_code_fx_rate", l_advertiser_default_currency_0)
          .getField("rate"),
        lit(1.0d)
      ).cast(DoubleType)
    ...
    l_currency_billing_exchange_rate =
      l_billing_exchange_rate_0.cast(DoubleType)
  ...
  def f_calc_derived_fields(
    l_ve:                                         Column,
    l_vi:                                         Column,
    l_vr:                                         Column,
    l_clicks:                                     Column,
    l_pixels:                                     Column,
    l_winning_creative_id:                        Column,
    global_variable_substituted_XR_BUSINESS_DATE: Column,
    global_variable_substituted_XR_BUSINESS_HOUR: Column
  ): Column =
    struct(
